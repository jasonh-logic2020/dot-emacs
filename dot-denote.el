;;; dot-denote --- set up denote
;;; Commentary:
;; A combination of prot’s denote config and jeremyf’s caategory
;; infrastructure from
;; https://github.com/jeremyf/dotemacs/blob/main/emacs.d/denote-emacs-configuration.org
...
(use-package denote
  ...
  :straight (denote :host nil :type git
                    :repo "https://git.sr.ht/~protesilaos/denote")
  :commands (denote-directory denote-file-prompt
                              denote--title-prompt denote-get-path-by-id)
  :bind
  ("C-c n j" . #'my-denote-journal)     ; our custom command
  ("C-c n n" . #'denote)
  ("C-c n N" . #'denote-type)
  ("C-c n d" . #'denote-date)
  ("C-c n f f" . #'jf/denote-find-file)
  ("C-c n s" . #'denote-subdirectory)
  ("C-c n t" . #'denote-template)
  ("C-c n i" . #'denote-link)           ; "insert" mnemonic
  ("C-c n I" . #'denote-link-add-links)
  ("C-c n b" . #'denote-link-backlinks)
  ("C-c n l f" . #'denote-link-find-file)
  ("C-c n l b" . #'denote-link-find-backlink)
  ("C-c n l l" . #'denote-link)
 ...
 - PROPERTY does not exist on the file"
	...
	(funcall fn initial-dir)
      (let* ((vc-dirs-ignores
		 ...
		 (project--files-in-directory
                  (f-join (denote-directory) sub-dir) vc-dirs-ignores))
	       jf/denote-subdirectories)))
	(funcall project-read-file-name-function
		 "Find file" all-files nil 'file-name-history))))

  (advice-add #'denote-file-prompt
              :around #'jf/denote-file-prompt '((name . "wrapper")))
  ...
  (cl-defmacro jf/denote-create-functions-for (&key domain key (create-fn nil))
    ...
    (let* ((finder-fn (intern (concat "jf/denote-find-file--" domain)))
	 ...
	 (bind-key (format "C-c n c %c" ,key)
                   (or ,create-fn ',default-create-fn))
	 (bind-key (format "C-c n s %c" ,key) ',finder-fn)
	 (defun ,finder-fn ()
	   ,finder-docstring
	   (interactive)
	   (let ((denote-directory (f-join (denote-directory) ,domain)))
	     (call-interactively #'jf/denote-find-file)))
	 (bind-key (format "C-c n n %c" ,key) ',inserter-fn)
	 (defun ,inserter-fn ()
	   ,inserter-docstring
	   (interactive)
	   (let ((denote-directory (f-join (denote-directory) ,domain)))
  ...
  (jf/denote-create-functions-for
   :domain "blog-posts" :key ?b)

  (jf/denote-create-functions-for :domain "dailies" :key ?d)

  (cl-defun jf/denote-create-epigraph
      (&key
...
Default the note’s title to the first NTH-WORDS of the BODY."
 ...
 NOTE: At present there is no consideration for uniqueness."
  ...
  (jf/denote-create-functions-for
   :domain "glossary" :key ?g :create-fn 'jf/denote-create-glossary-entry)
  ...
  ;; (message "%s" (jf/calculated-list-of-denote-known-keywords :from
  ;; "~/git/org/denote/glossary"))

  (jf/denote-create-functions-for :domain "melange" :key ?m)

  (jf/denote-create-functions-for :domain "people" :key ?p)

  (cl-defun jf/denote-create-indices-entry
      (&key
  ...
  (jf/denote-create-functions-for
   :domain "indices" :key ?i :create-fn 'jf/denote-create-indices-entry)

  (cl-defun jf/org-link-complete-link-for (parg &key scheme
                                                filter subdirectory)
    "Prompt for a SCHEME compatible `denote' with filename FILTER in the given SUBDIRECTORY.

Returns a string of format: \"SCHEME:<id>\" where <id> is an
`denote' identifier."
    (let* ((denote-directory (if subdirectory
				 (f-join (denote-directory)
					 (concat subdirectory "/"))
			       (denote-directory))))
      ;; This leverages a post v1.0.0 parameter of Denote See
      ;; https://git.sr.ht/~protesilaos/denote/commit/c6c3fc95c66ba093a266c775f411c0c8615c14c7
      (concat scheme
	      ":"
...
DESCRIPTION.  PROTOCOL is ignored."
    (let* ((prop-list (jf/denote-org-properties-from-id
		       ...
		       :properties (list "TITLE"
		                         property-name "GLOSSARY_KEY")))
	   (title (car (alist-get "TITLE" prop-list nil nil #'string=)))
	   (property (car (alist-get property-name prop-list nil
	                             nil #'string=)))
  ...
  (cl-defun jf/denote-link-ol-link-with-property
...
DESCRIPTION.  PROTOCOL is ignored."
    (let* ((prop-list (jf/denote-org-properties-from-id
		       ...
		       :properties (list "TITLE"
                                         property-name  "GLOSSARY_KEY")))
	   (title (car (alist-get "TITLE" prop-list nil nil #'string=)))
	   (property (car (alist-get
                           property-name prop-list nil nil #'string=)))
	   (key (car (alist-get "GLOSSARY_KEY" prop-list nil nil #'string=))))
      (cond
       ((or (eq format 'html) (eq format 'md))
	(if use_hugo_shortcode
  ...
  (org-link-set-parameters "date"
			   ...
			   ;; :store (lambda (jf/org-link-store-link-for
			   ;; :scheme "abbr"))
			   )
...
months; and most often year, month, and days."
  ...
  (cl-defun jf/denote-link-ol-epigraph-link-with-property
      (link description format protocol
	    &key
	    property-name
	    additional-hugo-parameters
...
TODO: Consider how to expand beyond blog support."
  ...
  (org-link-set-parameters
   ...
   :export (lambda (link description format protocol)
	     (jf/denote-link-ol-epigraph-link-with-property
              link description format protocol
	      :property-name "ABBR"
	      :additional-hugo-parameters "abbr=\"t\""))
   :face #'denote-faces-link
  ...
  (cl-defun jf/denote-link-ol-export
      (link description format protocol
 ...
 {{< glossary key=\"GLOSSARY_KEY\" >}}."
    (let* ((path-id (denote-link--ol-resolve-link-to-target link :path-id))
	   (title (jf/denote-org-property-from-id
                   :identifier link :property "TITLE"))
	  ...
	  (cond
	   ...
	   ((eq format 'ascii) (format "[%s] <denote:%s>" desc path))
           ;; NOTE 2022-06-16: May be tweaked further
           ;; ((eq format 'md) (format "[%s](%s)" desc url))
	   (t path))
	desc)))

  (advice-add #'denote-link-ol-export :override
              #'jf/denote-link-ol-export '((name . "wrapper")))
  ...
  (defun jf/denote-export-url-from-id (identifier)
    ...
    (when-let ((filename (denote-get-path-by-id identifier)))
      (when (string= (file-name-extension filename) "org")
	(with-current-buffer (find-file-noselect filename)
	  (let* ((props-plist (jf/org-global-props-as-plist
	                       :props-regexp
                               "\\(OFFER\\|ROAM_REFS\\|SAME_AS\\)")))
	    (cond
...
;;; Used as test.
  ...
  :custom ((denote-directory (expand-file-name "denote" org-directory))
	   ...
	   (denote-known-keywords
            (jf/calculated-list-of-denote-known-keywords
...
(use-package consult-notes
  ...
  :custom
  (consult-notes-sources (list))
  :commands (consult-notes
	     consult-notes-search-in-all-notes))
(provide 'dot-denote)

;;; dot-denote.el ends here
